...
![虚函数与虚表](./VirtualFunctionsAndVirtualTables/虚函数(1).jpg "虚函数与虚表")
![虚函数与虚表](./VirtualFunctionsAndVirtualTables/虚函数(2).jpg "虚函数与虚表")
...
![菱形虚拟继承内存图](./VirtualFunctionsAndVirtualTables/菱形虚拟继承内存图.png "菱形虚拟继承内存图")
* `B` 和 `C` 均有 虚表指针（第一个）和虚基表指针（第二个）
* 对于虚基表里面的内容， 它里面存储的是 `偏移量` ，第一个是 `-4` ，第二个是 `18` ，可见，第一个指针是为了找到该部分的起始位置，第二个指针是为了找到 `A` 的部分

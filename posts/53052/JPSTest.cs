...
public class PathfindingAlgorithmTest : MonoBehaviour
{
    ...
    private JPSNode[ , ] InitMap(int width, int height)
    {
        JPSNode[ , ] map = new JPSNode[height, width];
        // 随机创建地图障碍物
        for (int i = 0; i < height; i++)
        {
            for (int j = 0; j < width; j++)
            {
                JPSNode node = new JPSNode(i, j, Random.Range(0, 100) < 40 ? GlobalEnum.NodeStatu.Stop : GlobalEnum.NodeStatu.Walk);
                map[i, j] = node;
            }
        }
        return map;
